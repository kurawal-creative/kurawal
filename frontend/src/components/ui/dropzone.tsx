...
const DropZoneArea = forwardRef<HTMLDivElement, DropZoneAreaProps>(({ className, children, ...props }, forwardedRef) => {
	...
	return (
		...
		<div
			...
			{...props}
			aria-label="dropzone"
			className={cn("border-input bg-background ring-offset-background focus-visible:ring-ring flex items-center justify-center rounded-md border px-4 py-2 focus-visible:ring-2 focus-visible:ring-offset-2 focus-visible:outline-none", context.isDragActive && "animate-pulse bg-black/5", context.isInvalid && "border-destructive", className)}
		>
			{children}
		</div>
	);
...
const DropzoneFileListItem = forwardRef<HTMLLIElement, DropzoneFileListItemProps<unknown, unknown>>(({ className, ...props }, ref) => {
	...
	return (
		...
		>
			<li ref={ref} aria-label="dropzone-file-list-item" aria-describedby={isInvalid ? messageId : undefined} className={cn("bg-muted/40 flex flex-col justify-center gap-2 rounded-md px-4 py-2", className)}>
				{props.children}
			</li>
		</DropzoneFileListContext.Provider>
	);
...
const DropzoneTrigger = forwardRef<HTMLLabelElement, DropzoneTriggerProps>(({ className, children, ...props }, ref) => {
	...
	return (
		<label ref={ref} {...props} className={cn("bg-secondary ring-offset-background hover:bg-secondary/80 has-[input:focus-visible]:ring-ring cursor-pointer rounded-sm px-4 py-2 font-medium transition-colors focus-within:outline-none has-[input:focus-visible]:ring-2 has-[input:focus-visible]:ring-offset-2", className)}>
			{children}
			<input
...
const InfiniteProgress = forwardRef<HTMLDivElement, InfiniteProgressProps>(({ className, ...props }, ref) => {
	...
	return (
		<div ref={ref} role="progressbar" aria-valuemin={0} aria-valuemax={100} aria-valuetext={valueTextMap[props.status]} {...props} className={cn("bg-muted relative h-2 w-full overflow-hidden rounded-full", className)}>
			<div
				//   TODO: add proper done transition
				className={cn("bg-primary h-full w-full rounded-full", done ? "translate-x-0" : "animate-infinite-progress", error && "bg-destructive")}
			/>
		</div>
	);
});
